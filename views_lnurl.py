...
bleskomat_lnurl_router = APIRouter()


# Handles signed URL from Bleskomat ATMs and "action" callback of auto-generated LNURLs.
@bleskomat_lnurl_router.get("/u", name="bleskomat.api_bleskomat_lnurl")
async def api_bleskomat_lnurl(req: Request):
    try:
        ...
        if "signature" in query:
            ...
            # Signature provided. Use signature to verify that the URL was generated by
            # an authorized device. Later validate parameters, auto-generate LNURL,
            ...
            # Signature is valid.
            # In the case of signed URLs, the secret is deterministic based on
            # the API key ID and signature.
            ...
            if not lnurl:
                ...
                except LnurlValidationError as exc:
                    raise LnurlHttpError(str(exc), HTTPStatus.BAD_REQUEST) from exc
                # Create a new LNURL using query parameters provided in the signed URL.
